  ...
  <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <h2>Add Tution Type</h2>
        <label for="name">Tution Type Name:</label>
        <input class="form-control" type="text" v-model="tutionTypeName" required placeholder="Add New Tution Type Name">
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-danger" data-bs-dismiss="modal">Close</button>
          <button type="submit" class="btn btn-success">Add Tution Type</button>
        </div>
      </form>
      </div>
    </div>
    ...
    <div class="row">
        ...
        <div class="card-body d-flex align-items-center justify-content-between p-4">
          <h4 class="fw-semibold mb-0">Tution Type</h4>
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
              <li class="breadcrumb-item">
                <a class="text-muted text-decoration-none" href="index.html">Home</a>
              </li>
              <li class="breadcrumb-item" aria-current="page">Tution Type List</li>
            </ol>
          </nav>
        </div>
      </div>
      ...
      <div class="col-lg-12">
            ...
            <div class="row  pb-2">
              ...
              <div class="col-lg-2"></div>
                  ...
                  <div class="col-lg-2 mt-3 text-end">
                    ...
                    <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#exampleModal" style="font-size:16px; font-weight:bold">Add Tution Type</button>
                    </div>  
            </div>
          </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="col" style="font-weight: bold">Tution Type Name</th>
            <th scope="col" style="font-weight: bold">Action</th>
          </tr>
        </thead>
    
        <tbody>
      <tr v-for="(tution_type, index) in tution_types" :key="tution_type.id">
            <th scope="row">{{ calculateSLNumber(index) }}</th>
            <td data-title="Tution Type Name">{{ tution_type.name }}</td>
            <td data-title="Action" class="action">
  <button @click="deleteTutionType(tution_type.id)" class="btn bg-red"><i class="fa-regular fa-trash-can text-light" style="font-size: 16px;"></i></button>
    
      <button class="btn btn-primary" @click="editTutionType(tution_type)"><i class="fa-regular fa-pen-to-square text-light" style="font-size: 16px;"></i></button>
...
export default {
  ...
  mounted() {
    this.fetchTutionTypes();
  },
  methods: {
    submitForm() {
      this.$axios.post('/api/tution_types', { name: this.tutionTypeName })
        .then(response => {
          this.tutionTypeName = '';
          
          // After successful submission, refresh the divisions array
          this.fetchTutionTypes();
        })
        .catch(error => {
          console.error('Error adding Tution Types', error);
        });
    },
    fetchTutionTypes() {
        this.$axios.get('/api/tution_types')
          .then(response => {
            this.tution_types = response.data;
          })
          .catch(error => {
            console.error('Error fetching tution_types', error);
          });
      },

      editTutionType(tution_type) {
        // Redirect to the edit form or handle editing logic
        console.log('Editing tution_type:', tution_type);
      },
  
      deleteTutionType(id) {
        this.$axios.delete(`/api/tution_types/${id}`)
          .then(() => {
            this.fetchTutionTypes(); // Refresh the list after deletion
          })
          .catch(error => {
            console.error('Error deleting tution_type', error);
          });
      },
