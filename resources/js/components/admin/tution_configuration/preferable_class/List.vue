<!-- src/components/Preferable_List.vue -->
<template>
  <!-- sidebar -->
  <Sidebar />
  <!-- sidebar -->
  ...
  <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    <input type="text" class="form-control" v-model="className" required>

    <label for="name">Numarical Value:</label>
    <input type="text" class="form-control" v-model="numaricalValue" required>
    ...
    <div class="row">
        ...
        <div class="card-body d-flex align-items-center justify-content-between p-4">
          <h4 class="fw-semibold mb-0">Preferable Class List</h4>
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
              <li class="breadcrumb-item">
                <a class="text-muted text-decoration-none" href="index.html">Home</a>
              </li>
              <li class="breadcrumb-item" aria-current="page">Preferable Class List</li>
            </ol>
          </nav>
        </div>
      </div>
      ...
      <div class="col-lg-12">
            ...
            <div class="row  pb-2">
              ...
              <div class="col-lg-2"></div>
                  ...
                  <div class="col-lg-2 mt-3 text-end">
                    ...
                    <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#exampleModal" style="font-size:16px; font-weight:bold">Add Preferable Class</button>
                    </div>  
            </div>
          </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="col" style="font-weight: bold">Category Name</th>
            <th scope="col" style="font-weight: bold">Class Name</th>
            <th scope="col" style="font-weight: bold">Numarical Value</th>
            <th scope="col" style="font-weight: bold">Action</th>
          </tr>
        </thead>
    
        <tbody>
      <tr v-for="(preferable_class, index) in preferable_classes" :key="preferable_class.id">
            <th scope="row">{{ calculateSLNumber(index) }}</th>
            <td data-title="Division Name">{{ preferable_class.category ? preferable_class.category.title : 'No Category' }}</td>
            <td data-title="Preferable Class Name">{{ preferable_class.class_name }}</td>
            <td data-title="Numarical Value">{{ preferable_class.numaric_value }}</td>
            <td data-title="Action" class="action">
    
            <button @click="deletePreferableClass(preferable_class.id)" class="btn bg-red"><i class="fa-regular fa-trash-can text-light" style="font-size: 16px;"></i></button>
      <button class="btn btn-primary" @click="editPreferableClass(preferable_class)"><i class="fa-regular fa-pen-to-square text-light" style="font-size: 16px;"></i></button>
    </td>
    </tr>
...
<script>
 import Sidebar from '@/components/admin/Sidebar.vue';
  ...
  methods: {
    ...
    submitForm() {
      this.$axios.post('/api/preferable_classes', {
        class_name: this.className,
        numaric_value: this.numaricalValue,
        ...
        .catch(error => {
          console.error('Error adding Preferable Class', error);
        });
    },

    fetchPreferableClasses() {
      this.$axios.get('/api/preferable_classes')
        .then(response => {
          this.preferable_classes = response.data;
        })
        .catch(error => {
          console.error('Error fetching preferable classes', error);
        });
    },
    editPreferableClass(preferable_class) {
      // Redirect to the edit form or handle editing logic
      console.log('Editing preferable classes:', preferable_class);
    },
    deletePreferableClass(id) {
      this.$axios.delete(`/api/preferable_classes/${id}`)
        .then(() => {
          this.fetchPreferableClasses(); // Refresh the list after deletion
        })
        .catch(error => {
          console.error('Error deleting preferable class', error);
        });
    },
