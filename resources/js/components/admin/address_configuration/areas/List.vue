<!-- src/components/AreaList.vue -->
  <template>
    ...
    <select class="form-control" v-model="selectedDistrict" required>
      <option v-for="district in filteredDistricts" :key="district.id" :value="district.id">{{ district.name }}</option>
    </select>

    <label for="name">Area Name:</label>
    <input type="text" class="form-control" v-model="areaName" required placeholder="Area Name">
      ...
      <div class="row">
          ...
          <div class="card-body d-flex align-items-center justify-content-between p-4">
            <h4 class="fw-semibold mb-0">Area List</h4>
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item">
                  <a class="text-muted text-decoration-none" href="index.html">Home</a>
                </li>
                <li class="breadcrumb-item" aria-current="page">Area List</li>
              </ol>
            </nav>
          </div>
        </div>
      ...
      <div class="row">
                ...
                <div class="col-lg-12">
                  <div class="row">
                    ...
                    <div class="col-lg-3 mt-3 text-end">
                      ...
                      <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#exampleModal" style="font-weight: bold;font-size:16px">Add Area</button>
                      </div>  
        ...
        <tr v-for="(area, index) in areas" :key="area.id">
              ...
              <td data-title="Division Name">{{ area.division ? area.division.name : 'No Division' }}</td>
              <td data-title="District Name">{{ area.district ? area.district.name : 'No District' }}</td>
              <td data-title="Area Name">{{ area.name }}</td>
              <td data-title="Action" class="action">
      
             <button class="btn bg-red" @click="deleteArea(area.id)"><i class="fa-regular fa-trash-can text-light" style="font-size: 16px;"></i></button>
      
        <button class="btn btn-primary" @click="editArea(area)"><i class="fa-regular fa-pen-to-square text-light" style="font-size: 16px;"></i></button>
  ...
  export default {
    ...
    submitForm() {
      ...
      deleteArea(id) {
        this.$axios.delete(`/api/areas/${id}`)
          .then(() => {
            this.fetchAreas(); // Refresh the list after deletion
          })
          .catch(error => {
