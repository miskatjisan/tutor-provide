<!-- src/components/DistrictList.vue -->
<template>
  <!-- sidebar -->
  <Sidebar />
  <!-- sidebar -->
  ...
  <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    <select v-model="selectedDivision" class="form-control" required>
      <option v-for="division in divisions" :key="division.id" :value="division.id">{{ division.name }}</option>
    </select>

    <label for="name">District Name:</label>
    <input type="text" class="form-control" v-model="districtName" required>
    ...
    <div class="row">
        ...
        <div class="card-body d-flex align-items-center justify-content-between p-4">
          <h4 class="fw-semibold mb-0">District List</h4>
    ...
    <div class="row">
            ...
            <div class="row  pb-2">
              ...
              <div class="col-lg-2"></div>
                  ...
                  <div class="col-lg-2 mt-3 text-end">
                    ...
                    <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#exampleModal" style="font-size:16px; font-weight:bold">Add District</button>
                    </div>  
            </div>
          </div>
      ...
      <tr v-for="(district, index) in districts" :key="district.id">
            ...
            <td data-title="Division Name">{{ district.division ? district.division.name : 'No Division' }}</td>
            <td data-title="District Name">{{ district.name }}</td>
            <td data-title="Action" class="action">
    
            <button @click="deleteDistrict(district.id)" class="btn bg-red"><i class="fa-regular fa-trash-can text-light" style="font-size: 16px;"></i></button>
      <button class="btn btn-primary" @click="editDistrict(district)"><i class="fa-regular fa-pen-to-square text-light" style="font-size: 16px;"></i></button>
    </td>
    </tr>
...
<script>
 import Sidebar from '@/components/admin/Sidebar.vue';
  ...
  methods: {
    ...
    submitForm() {
      this.$axios.post('/api/districts', {
        name: this.districtName,
        division_id: this.selectedDivision,
      })
        .then(response => {
          this.$emit('districtAdded', response.data);
          this.districtName = '';
    ...
    deleteDistrict(id) {
      this.$axios.delete(`/api/districts/${id}`)
        .then(() => {
          this.fetchDistricts(); // Refresh the list after deletion
        })
        .catch(error => {
