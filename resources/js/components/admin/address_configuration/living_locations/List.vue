<!-- src/components/LivingLocationList.vue -->
<template>
<!-- sidebar -->
<Sidebar />
<!-- sidebar -->
    ...
    <!-- <div style="margin-left: 50% !important">
      <h2>Living Location List</h2>
    <button class="btn bg-orange">Add Living Location</button>
      <div v-for="location in livingLocations" :key="location.id">
        {{ location.name }} - {{ location.area ? location.area.name : 'No Area' }} - {{ location.district ? location.district.name : 'No District' }} - {{ location.division ? location.division.name : 'No Division' }}
    ...
    <select class="form-control" v-model="selectedDivision" @change="filterDistricts" required>
      <option v-for="division in divisions" :key="division.id" :value="division.id">{{ division.name }}</option>
    </select>

    <label for="district">Select District:</label>
    <select class="form-control" v-model="selectedDistrict" @change="filterAreas" required>
      <option v-for="district in filteredDistricts" :key="district.id" :value="district.id">{{ district.name }}</option>
    </select>

    <label for="area">Select Area:</label>
    <select class="form-control" v-model="selectedArea" required>
      <option v-for="area in filteredAreas" :key="area.id" :value="area.id">{{ area.name }}</option>
    </select>

    <label for="name">Living Location Name:</label>
    <input  class="form-control" type="text" v-model="locationName" required placeholder="Living Location Name">
      ...
      <div class="row">
          <div class="col-lg-12 d-flex align-items-stretch">
            <div class="card shadow-none position-relative overflow-hidden mb-4 w-100">
          <div class="card-body d-flex align-items-center justify-content-between p-4">
            <h4 class="fw-semibold mb-0">Living Location List</h4>
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item">
                  <a class="text-muted text-decoration-none" href="index.html">Home</a>
                </li>
                <li class="breadcrumb-item" aria-current="page">Living Location List</li>
              </ol>
            </nav>
          </div>
        </div>
      ...
      <option v-for="area in filteredAreas" :key="area.id" :value="area.id">{{ area.name }}</option>
                    ...
                    <div class="col-lg-2 text-end mt-3">
                     
                      <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#exampleModal" style="font-size:16px; font-weight:bold">Add Living Location</button>
                      </div>  
          ...
          <thead>
            <tr>
              <th scope="col" style="width: 5%; font-weight:bold">SL</th>
              <th scope="col" style="font-weight:bold">Division Name</th>
              <th scope="col" style="font-weight:bold">District Name</th>
              <th scope="col" style="font-weight:bold">Area Name</th>
              <th scope="col" style="font-weight:bold">Living Location Name</th>
              <th scope="col" style="font-weight:bold">Action</th>
            </tr>
          </thead>
      
          <tbody>
        <tr v-for="(location, index) in livingLocations" :key="location.id">
              <th scope="row">{{ calculateSLNumber(index) }}</th>
              <td data-title="Division Name">{{ location.division ? location.division.name : 'No Division' }}</td>
              <td data-title="District Name">{{ location.district ? location.district.name : 'No District' }}</td>
              <td data-title="Area Name">{{ location.area ? location.area.name : 'No Area' }}</td>
              <td data-title="Living Location Name">{{ location.name }}</td>
              <td data-title="Action" class="action">
                <button class="btn bg-red" @click="deleteLocation(location.id)"><i class="fa-regular fa-trash-can text-light" style="font-size: 16px;"></i></button>
        <button class="btn btn-primary" @click="editLocation(location)"><i class="fa-regular fa-pen-to-square text-light" style="font-size: 16px;"></i></button>
 ...
 import Sidebar from '@/components/admin/Sidebar.vue';
  export default {
    ...
    mounted() {
      ...
      this.fetchDistricts();
      this.fetchAreas();
      this.fetchLivingLocations();
    },
    methods: {
      fetchDivisions() {
      this.$axios.get('/api/divisions')
        .then(response => {
    ...
    filterDistricts() {
      this.filteredDistricts = this.districts.filter(district => district.division_id === this.selectedDivision);
      this.selectedDistrict = null;
      this.filteredAreas = []; 
    },
    filterAreas() {
      this.filteredAreas = this.areas.filter(area => area.district_id === this.selectedDistrict);
    },
    submitForm() {
      this.$axios.post('/api/living-locations', { 
        ...
        .then(response => {
          this.$emit('locationAdded', response.data);
          this.locationName = '';
          this.selectedDivision = null;
          this.selectedDistrict = null;
          this.selectedArea = null;
        // After successful submission, refresh the district array
          this.fetchLivingLocations();

        })
        .catch(error => {
          console.error('Error adding living location', error);
        });
    },


      fetchLivingLocations() {
        this.$axios.get('/api/living-locations')
          .then(response => {
            this.livingLocations = response.data;
          })
          .catch(error => {
            console.error('Error fetching living locations', error);
          });
      },
      editLocation(location) {
        // Redirect to the edit form or handle editing logic
        console.log('Editing living location:', location);
      },
      deleteLocation(id) {
        this.$axios.delete(`/api/living-locations/${id}`)
          .then(() => {
            this.fetchLivingLocations(); // Refresh the list after deletion
          })
          .catch(error => {
            console.error('Error deleting living location', error);
          });
      },

      calculateSLNumber(index) {
      return index + 1;
    },
